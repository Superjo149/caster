            ...
            zoneResult.zones.forEach(function(zone) {
              ...
              if (!exists) {
                ...
                log(
                  'debug',
                  'discoverZones()',
                  'doesnt exist, pushing: ' + zone.id
                );
              } else {
                log(
                  'debug',
                  'discoverZones()',
                  'exists, pushed already: ' + zone.id
                );
              }
            });
          });
    ...
    discover('googlezone')
      .then(devices => {
        log(
          'debug',
          'discoverTimes()',
          'discover("googlezone") done times: ' +
            times +
            ', zoneResults: ' +
            zoneResults +
            ', typeof: ' +
        ...
        if (times > 1) {
          log(
            'debug',
            'discoverTimes()',
            'newZoneResults: ' + zoneResults + ', new times: ' + (times - 1)
          );
          ...
          discover('googlecast')
            ...
            .catch(errorMessage => {
              log(
                'error',
                'discoverTimes()',
                'discover("googlecast") failed: ' + errorMessage
              );
              resolve(zoneResults);
            });
        }
      ...
      .catch(errorMessage => {
        log(
          'error',
          'discoverTimes()',
          'discover("googlezone") failed: ' + errorMessage
        );
        reject(errorMessage);
      });
  });
      ...
      if (getNetworkIp) {
        ...
        browser.on('update', function(service) {
          ...
          log(
            'debug',
            'discover()',
            'update received, service: ' + JSON.stringify(service)
          );
          if (target == 'googlecast' && service.type[0].name == target) {
            ...
            if (
              !duplicateDevice(discovered, currentDevice) &&
              currentDevice.name != null
            ) {
              log(
                'debug',
                'discover()',
                'found device: ' + JSON.stringify(currentDevice)
              );
              ...
              if (!deviceExists(currentDevice.id)) {
                log(
                  ...
                  'added device name: ' +
                    currentDevice.name +
                    ', address: ' +
                    JSON.stringify(currentDevice.address),
                  currentDevice.id
                );
                devices.push(
                  new CastDevice(
          ...
          if (target == 'googlezone' && service.type[0].name == target) {
            ...
            log(
              'debug',
              'discover()',
              'found googlezone: ' + JSON.stringify(currentGroupMembership)
            );
...
function updateExistingCastDeviceAddress(discoveredDevice) {
  log(
    ...
    'discoveredDevice: ' + JSON.stringify(discoveredDevice),
    discoveredDevice.id
  );
  if (deviceExists(discoveredDevice.id)) {
    ...
    if (discoveredDevice.address.host && discoveredDevice.address.port) {
      if (
        castDevice.address.host != discoveredDevice.address.host ||
        castDevice.address.port != discoveredDevice.address.port
      ) {
        log(
          'info',
          'updateExistingCastDeviceAddress()',
          'updating address from: ' +
            castDevice.address.host +
            ':' +
            castDevice.address.port +
            ' to: ' +
            discoveredDevice.address.host +
            ':' +
            discoveredDevice.address.port,
          discoveredDevice.id
        );
        ...
        if (castDevice.link == 'connected') {
          ...
          castDevice.event.once('linkChanged', function() {
            log(
              'info',
              'updateExistingCastDeviceAddress()',
              'once linkChanged: ' + castDevice.link,
              castDevice.id
            );
            castDevice.address = {
...
function createGoogleZones(discoveredZones) {
  log(
    'debug',
    'createGoogleZones()',
    'discoveredZones: ' + JSON.stringify(discoveredZones)
  );
      ...
      device.groups.forEach(function(groupId) {
        ...
        if (groupExists) {
          log(
            'debug',
            'createGoogleZones()',
            'groupExists: ' + JSON.stringify(groupExists)
          );
          ...
          log(
            'debug',
            'createGoogleZones()',
            'group doesnt exist: ' + groupExists
          );
          zones.push({
            id: groupId,
            members: [device.id]
