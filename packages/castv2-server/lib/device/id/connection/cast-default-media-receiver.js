    ...
    try {
      ...
      media.forEach(function(element, index) {
        ...
        logger.log(
          'info',
          'CastDevice.playMedia()',
          'mediaElement: ' + JSON.stringify(mediaElement),
          that.id
        );
      ...
      GoogleTts.replace(mediaList) //TODO:
        .then(newMediaList => {
          logger.log(
            'info',
            'CastDevice.playMedia()',
            'newMediaList: ' + JSON.stringify(newMediaList),
            that.id
          );

          this._castv2Client.connect(this._address, () => {
            this._castv2Client.launch(DefaultMediaReceiver, function(
              err,
              player
            ) {
              player.queueLoad(
          ...
          setTimeout(() => {
            try {
              this._castv2Client.close();
            } catch (e) {
              logger.log(
                'error',
                'CastDevice.playMedia()',
                'this._castv2Client.close() exception: ' + e,
                that.id
              );
            }
          }, 5000);
