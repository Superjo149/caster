...
class CastMedia extends events {
      ...
      try {
        ...
        this._client.connect(this._address, function() {
          ...
          try {
            ...
            that._media = that._client.createChannel(
              'sender-0',
              that._sessionId,
  ...
  parse(mediaStatus) {
    try {
      logger.log(
        'info',
        'CastMedia.parse()',
        'mediaStatus: ' + JSON.stringify(mediaStatus),
        this._id
      );
        ...
        if (mediaStatus.status[0]) {
          if (mediaStatus.status[0].media) {
            if (mediaStatus.status[0].media.metadata) {
              let metadataType =
                mediaStatus.status[0].media.metadata.metadataType;
              if (metadataType <= 1) {
                this.status = {
  ...
  play() {
    logger.log(
      'info',
      'CastMedia.play()',
      'this._media:' + this._media + 'this.link: ' + this.link,
      this.id
    );
    if (
      this._media != null &&
