...
class CastDevice extends events {
  ...
  set address(value) {
    ...
    if (
      this._address.host !== value.host ||
      this._address.port !== value.port
    ) {
      this._address = value;
      ...
      logger.log(
        ...
        'changed _address: ' +
          ...
          JSON.stringify(this._castReceiver._address) +
          JSON.stringify(this._castMedia._address),
        this.id
      );
  ...
  set status(status) {
    ...
    for (let key in status) {
      ...
      if (
        ...
        !this.isGroupPlaybackLocked(status, key)
      ) {
        this._status[key] = value;
        changed = true;
      }
  ...
  isGroupPlaybackLocked(status, key) {
    //TODO: does not lock if groupPlayback is set,
    let locked = false;
    if (
      key === 'groupPlayback' ||
      key === 'application' ||
      ...
      logger.log(
        'info',
        'CastDevice.isGroupPlaybackLocked()',
        'is media key: ' + key,
        this.id
      );
      if (this._status.groupPlayback.on !== false) {
        ...
        if (status.hasOwnProperty('groupPlayback')) {
          logger.log(
            ...
            'CastDevice.isGroupPlaybackLocked()',
            'status.hasOwnProp: ' +
              status.hasOwnProperty('groupPlayback') +
              ' this._status.groupPlayback: ' +
              JSON.stringify(this._status.groupPlayback),
            this.id
          );
          if (status.groupPlayback.from === this._status.groupPlayback.from) {
            logger.log(
              'info',
              'CastDevice.isGroupPlaybackLocked()',
              'status.groupPlayback.from: ' +
                status.groupPlayback.from +
                '===this._status.groupPlayback.from: ' +
                this._status.groupPlayback.from,
              this.id
            );
            locked = false;
    ...
    logger.log(
      'info',
      'CastDevice.isGroupPlaybackLocked()',
      'key: ' + key + ', status[key]: ' + status[key] + ', lock: ' + locked,
      this.id
    ); //TODO: sometimes: key: groupPlayback, locked: false and still the new groupPlayback is not written
    return locked;
  }
