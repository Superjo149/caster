...
class Config {
  ...
  static getLatestVersion() {
    return new Promise(function(resolve, reject) {
      fetch(
        'https://raw.githubusercontent.com/vervallsweg/cast-web-api/master/package.json'
      )
        .then(function(res) {
          return res.json();
        })
        .then(function(json) {
          require('../log/logger').log(
            'debug',
            'getLatestVersion()',
            'JSON received: ' + JSON.stringify(json)
          );
          try {
  ...
  static getConfig() {
    return new Promise(resolve => {
      ...
      Promise.all([
        Config.getLatestVersion().catch(() => {
          return 'Error';
        }),
        AssistantSetup.checkSetup().catch(() => {
          return { id: false, secret: false, token: false };
        })
      ])
        .then(values => {
