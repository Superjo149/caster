  ...
  this.event.on('statusChange', function() {
    if (that.members) {
      if (that.status.application) {
        if (
          that.status.application != 'Backdrop' &&
          that.status.application != ''
        ) {
          //TODO: use isIdleScreen
          log(
            'info',
            'connectGroupMembersInit()',
            'statusChange is group',
            that.id
          );
          ...
          log(
            'error',
            'connectGroupMembersInit()',
            'statusChange is APP no title group',
            that.id
          );
  ...
  this.event.on('groupChange', function(operation, zone) {
    ...
    if (zone.members) {
      log(
        level,
        'on groupChange()',
        operation + ' members: ' + zone.members,
        zone.id
      );
      if (operation == 'remove') {
          ...
          if (deviceExists(memberId)) {
            ...
            if (member.groupPlayback) {
              log(
                level,
                'on groupChange()',
                'resetting groupPlayback for member: ' + memberId,
                zone.id
              );
              syncGroupMemberStatus(
                {
                  application: '',
                  status: '',
                  title: '',
      ...
      if (operation == 'add') {
        if (deviceExists(zone.id)) {
          ...
          log(
            level,
            'on groupChange()',
            'evaluating groupPlayback for new members: ' + zone.members,
            zone.id
          );
      ...
      if (group && group != '' && deviceExists(group)) {
        ...
        if (castDeviceGroup.link == 'disconnected') {
          log(
            'info',
            'connectGroupMembers()',
            'castDevice.connect(), connecting group: ' +
              group +
              ', castDeviceGroup.link: ' +
              castDeviceGroup.link,
            castDevice.id
          );
...
function resetGroupMemberStatus(castDevice) {
  if (castDevice.members) {
    if (castDevice.members[0]) {
      if (deviceExists(castDevice.members[0])) {
        ...
        if (member.status) {
          if (member.status.groupPlaybackOn == castDevice.id) {
            ...
            log(
              'info',
              'resetGroupMemberStatus()',
              'not resetting, getDevice(castDevice.members[0]).status.groupPlaybackOn: ' +
                getDevice(castDevice.members[0]).status.groupPlaybackOn +
                ' != ' +
                castDevice.id,
              castDevice.id
            );
...
function syncGroupMemberStatus(status, members, id, groupPlayback) {
  log(
    'info',
    'syncGroupMemberStatus(' + groupPlayback + ')',
    'to members: ' + members + ', new status: ' + JSON.stringify(status),
    id
  );
    ...
    if (castDevice.members) {
      zones.members.forEach(function(memberId) {
        if (castDevice.members.indexOf(memberId) < 0) {
          ...
          castDevice.event.emit('groupChange', 'add', {
            id: castDevice.id,
            members: [memberId]
          });
        }
      ...
      castDevice.members.forEach(function(memberId) {
        if (zones.members.indexOf(memberId) < 0) {
          ...
          castDevice.event.emit('groupChange', 'remove', {
            id: castDevice.id,
            members: [memberId]
          });
        }
