...
class ReconnectionManager {
  ...
  evaluate() {
    ...
    if (this._device.link === 'disconnected') {
      if (this._interval == null) {
        ...
        if (configuration._reconnectTimeout > 0) {
          logger.log(
            'info',
            'ReconnectionManager.evaluate()',
            'setting interval, timeout: ' + configuration.reconnectTimeout,
            this._device.id
          );
      ...
      logger.log(
        'info',
        'ReconnectionManager.evaluate()',
        'this._device.link !== disconnected',
        this._device.id
      );
      if (this._interval != null) {
        logger.log(
  ...
  reconnect() {
    ...
    if (
      this._device.link !== 'connected' &&
      this._device.link !== 'connecting'
    ) {
      logger.log(
        'info',
