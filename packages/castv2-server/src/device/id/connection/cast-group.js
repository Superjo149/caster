  ...
  syncStatus() {
    if (this._members) {
      if (this._master.link === 'connected' && this._master.sessionId != null) {
        logger.log(
          'info',
          'CastGroup.syncStatus()',
          'syncing group this._master.sessionId' + this._master.sessionId,
          this._master.id
        );
        ...
        logger.log(
          'info',
          'CastGroup.syncStatus()',
          'used to be group, remove & reset',
          this._master.id
        );

        this.setStatus({
          groupPlayback: { on: false, from: this._master.id },
          application: '',
          status: '',
          title: '',
          subtitle: '',
      ...
      logger.log(
        'info',
        'CastGroup.syncStatus()',
        'has no members, removing',
        this._master.id
      ); //TODO: wtf? If it has no members, what are we iterating over for delete??
