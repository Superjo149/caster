...
class CastReceiver extends events {
  ...
  connect() {
    try {
      logger.log(
        'info',
        'CastReceiver.connect()',
        'host: ' + this._address.host + ', port: ' + this._address.port,
        this._id
      );
      let that = this;
      ...
      this._client.connect(this._address, function() {
        try {
          that._connection = that._client.createChannel(
            'sender-0',
            'receiver-0',
            'urn:x-cast:com.google.cast.tp.connection',
            'JSON'
          );
          that._heartbeat = that._client.createChannel(
            'sender-0',
            'receiver-0',
            'urn:x-cast:com.google.cast.tp.heartbeat',
            'JSON'
          );
          that._receiver = that._client.createChannel(
            'sender-0',
            'receiver-0',
          ...
          that._heartBeatIntervall = setInterval(function() {
            ...
            try {
              ...
              logger.log(
                'error',
                'CastReceiver.connect()',
                'heartBeatIntervall exception: ' + e,
                that._id
              );
              that.disconnect();
            }
      ...
      this._client.on('error', function(err) {
        logger.log(
          'error',
          'CastReceiver.connect()',
          'castDevice.castConnectionReceiver.client error: ' + err,
          that._id
        );
        that.disconnect();
      });
  ...
  parse(receiverStatus) {
    try {
      logger.log(
        'info',
        'CastReceiver.parse()',
        'receiverStatus: ' + JSON.stringify(receiverStatus),
        this._id
      );
        ...
        if (receiverStatus.status.applications) {
          if (receiverStatus.status.applications[0].isIdleScreen !== true) {
            logger.log(
              ...
              'CastReceiver.parse()',
              'isIdleScreen: ' +
                receiverStatus.status.applications[0].isIdleScreen +
                ', sessionId changed to: ' +
                receiverStatus.status.applications[0].sessionId +
                ', from: ' +
                this._sessionId,
              this._id
            );
          ...
          if (receiverStatus.status.applications[0].displayName) {
            this.status = {
              application: receiverStatus.status.applications[0].displayName
            };
          }
        } else {
  ...
  disconnect() {
    logger.log(
      'info',
      'CastReceiver.disconnect()',
      'host: ' +
        this._address.host +
        ', port: ' +
        this._address.port +
        ', heartbeat: ' +
        this._heartBeatIntervall,
      this._id
    );
    try {
