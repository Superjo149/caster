...
class CastManager {
  ...
  getDeviceConnected(id) {
    return new Promise((resolve, reject) => {
      ...
      if (targetDevice) {
        //TODO: check return value if doesn't exist
        if (targetDevice.link === 'connected') {
          ...
          targetDevice.event.once('linkChanged', () => {
            Logger.log(
              'debug',
              'getDeviceConnected()',
              'once linkChanged: ' + targetDevice.link,
              id
            );
            resolve(targetDevice);
          });
  ...
  addDevice(device) {
    if (!this._devices.has(device.id)) {
      //TODO: issue if device down -> device up with new address
  ...
  removeDevice(id) {
    ...
    if (this._devices.has(id)) {
      ...
      Logger.log(
        'info',
        'removeDevice()',
        'deviceExists: ' + this._devices.has(id),
        id
      );
    }
  }
