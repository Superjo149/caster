...
class AssistantSetup {
  ...
  static setToken(oAuthCode) {
    return new Promise(function(resolve, reject) {
      AssistantSetup.readClientSecretJSON()
        .then(existing => {
          if (
            existing.installed.client_id &&
            existing.installed.client_secret &&
            existing.installed.redirect_uris
          ) {
            try {
              const oauthClient = new OAuth2(
                existing.installed.client_id,
                existing.installed.client_secret,
                existing.installed.redirect_uris[0]
              );
  ...
  static getAbsolutePath() {
    let path = require
      .resolve('../assistant/google-assistant')
      .substring(
        0,
        require.resolve('../assistant/google-assistant').length - 20
      );
    console.log(path);
    return path;
  }
